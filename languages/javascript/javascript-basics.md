...
**try/catch** - конструкция для перехвата и обработки ошибок, чтобы скрипт не падал, а продолжал работу.

**finally** - блок выполняется всегда, была ошибка или нет (для завершающих действий).

**throw** - генерация собственной ошибки.

```js
try {
  console.log("Работает");
  неСуществующаяФункция(); // Ошибка!
  console.log("Не выполнится");
} catch (error) {
  // Блок catch выполняется только при ошибке
  console.log("Произошла ошибка:", error.message); // Произошла ошибка: неСуществующаяФункция is not defined
  console.log(error.name); // ReferenceError
  console.log(error.message); // неСуществующаяФункция is not defined
  console.log(error.stack); // ReferenceError: ... (стек вызовов)
  throw new Error("Генерация собственной ошибки"); // Вылетит наружу после finally
} finally {
  console.log("Завершение"); // Выполнится в любом случае
}

// Вывод:
// Работает
// Произошла ошибка: неСуществующаяФункция is not defined
// ReferenceError
// неСуществующаяФункция is not defined
// (стек ошибки)
// Завершение
// Error: Генерация собственной ошибки
```

В `catch` попадает объект ошибки `error` с основными свойствами:

* **name** - тип ошибки (`SyntaxError`, `TypeError`, `ReferenceError` и т.д.).
