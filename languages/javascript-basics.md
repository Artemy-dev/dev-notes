...
* Высокоуровневый со сборщиком мусора. Сборщик мусора (GC) - это программа, которая сама удаляет из памяти ненужные данные (неиспользуемые переменные и объекты).
* JIT компилируемый
...
* Высокоуровневые языки (автоматическое управление ресурсами). Пример: `JavaScript`, `Go`, `Python`.
* Низкоуровневые (ручное управление ресурсами). Пример: `С`, `Rust`.

---

### JIT (Just-In-Time) компилируемый

Код сначала читают построчно (работает как интерпретируемая), наиболее часто используемые участки кода компилируются в машинный код во время выполнения. Это дает производительность близкую к скомпилированным языкам, сохраняя гибкость интерпретируемых.

* **Компиляция (когда код целиком переводится в машинный код до запуска, создавая исполняемый файл):** Исходный код → Машинный код → Исполнение
* **Интерпретация (когда код выполняется построчно другой программой (интерпретатором) без предварительного преобразования в машинный код):** Исходный код → Исполнение
* **JIT:** Исходный код → Машинный код (часто используемые участки)  → Исполнение

---

### Мультипарадигмальный

Поддерживает несколько подходов к написанию кода:

* Объектно-ориентированный (работа с объектами и классами)
* Функциональный (функции как значения, замыкания, чистые функции)
* Процедурный (последовательное выполнение инструкций)

```javascript
// Процедурный стиль (шаг за шагом)
let x = 5;
x = x + 10;
console.log(x);

// Функциональный стиль (чистые функции, без изменений данных)
const addTen = x => x + 10;
console.log(addTen(5));

// ООП стиль (объект с данными и методами)
...
new NumberProcessor(5).addTen().print();
```
...
Имеет один главный поток выполнения и один стек вызовов. Операции выполняются последовательно, но благодаря Event Loop (цикл событий) может обрабатывать асинхронные задачи (таймеры, запросы, ввод/вывод) без блокировки основного потока.

---

### Динамически типизированный

Тип данных определяется автоматически во время выполнения. Переменная может хранить значения разных типов в процессе работы программы.

```javascript
let x = 5;        // число
x = "строка";     // теперь строка
x = true;         // теперь булево
x = {name: "A"};  // теперь объект
```
...
const pi = 3.14;   // const - ключевое слово при объявлении переменной c защитой от перезаписи
console.log(pi);   // pi = 1.2; - попытка перезаписать выведет ошибку TypeError: Assignment to constant variable.
```

В повседневной разработке лучше использовать `const` по умолчанию. Изменяйте на `let` только тогда, когда точно понимаете, что переменная будет переназначаться (переприсваиваться).
...
Когда вы пишете `const name = 'Tim';`, вы говорите себе и другим разработчикам: "Это имя никогда не изменится на протяжении всего скрипта". Когда вы видите `let age = 30;`, вы понимаете: "Ага, где-то ниже возраст может поменяться (`age = 31`)". Это упрощает чтение кода. Если везде использовать `let`, вы теряете этот сигнал и приходится постоянно держать в голове: "А может ли эта переменная измениться?"
